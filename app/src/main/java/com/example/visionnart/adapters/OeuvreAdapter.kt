...
class OeuvreAdapter(val oeuvres: ArrayList<Oeuvre>) :
    androidx.recyclerview.widget.RecyclerView.Adapter<OeuvreAdapter.ViewHolder>() {

    private lateinit var context: Context
    private lateinit var mOnItemClickListener: OnItemClickListener

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        val v = LayoutInflater.from(parent.context).inflate(R.layout.item_oeuvres, parent, false)
        context = v.context
        return ViewHolder(v)
    }
    ...
    inner class ViewHolder(itemView: View) : androidx.recyclerview.widget.RecyclerView.ViewHolder(itemView) {
        fun bindItems(oeuvre: Oeuvre) {
            ...
            Picasso.get().load(oeuvre.img_oeuvre).into(itemView.Photooeuvres)
            ...
            itemView.itemOeuvreName.text = name
        }
    }
